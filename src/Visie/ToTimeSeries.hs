...
fill :: Monoid a => UTCTime -> Timestamped a
...
convert interval elements = sampler sorted
  where sorted = sortOn getTime elements
        ...
        consume :: Monoid a => [UTCTime] -> [Timestamped a] -> [Timestamped a]
        consume (t:ts) [] = []
...
convertFill :: Monoid a => UTCTime -> NominalDiffTime -> [Timestamped a] -> [Timestamped a]
